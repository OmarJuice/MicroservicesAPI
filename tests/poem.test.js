...
describe('POST /api/poem', () => {
    it('should return a poem if text is sent', (done) => {
        request(app)
            .post('/api/poem')
            .send({
                poem: {text, numLines}
            ...
            .expect(200)
            .expect((res) => {
                expect(typeof res.body.numUniqueWords).toBe('number')
                expect(res.body.poem.length).toBe(numLines)
            })
            .end(done)
    })
    it('should return 400 if no text is sent', (done) => {
        request(app)
            .post('/api/poem')
            .send()
            .expect(400)
